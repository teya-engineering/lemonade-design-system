---
name: ktdoc-quality
description: Use when writing, editing, or reviewing KDoc comments in Kotlin code (class headers, summary lines, @param/@return tags). Enforces concise, active, linked KDoc. Adapted from androidx/androidx .agents/skills/ktdoc_quality (Apache 2.0).
---

> **Lemonade project note (local addition — everything below this block is upstream AndroidX content).**
>
> - **This skill governs how a KDoc reads once it exists. Whether a KDoc should exist at all is decided by `.claude/skills/comment-review/references/comment-rules.md`**: the published API surface earns KDoc when it adds what the signature can't (units, ranges, nullability meaning, which errors come back); internal declarations almost never qualify. Never ADD a KDoc to satisfy this skill — that inverts the comments rule. Component KDoc with a `## Usage` block on the published modules is the canonical downstream reference — apply these style rules to its prose, but never delete or shrink the `## Usage` example itself.
> - Every violation of the rules below on a KDoc the diff adds or edits is a **blocking** finding. There are no KDoc "warnings".
> - `@sample` is AndroidX-specific sample infrastructure — Lemonade has no equivalent; component KDoc uses `## Usage` blocks with short inline code examples instead. That is the house pattern; keep it.
> - The "AndroidX" framing and Android-platform link examples below are upstream's; the rules apply unchanged to Lemonade Kotlin code (commonMain included — see upstream safeguard §5 on multiplatform cleanliness). For Swift doc comments, apply the same spirit via the `comment-review` skill's `references/comment-rules.md` § Formatting.
