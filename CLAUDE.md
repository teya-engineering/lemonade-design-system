...
Everything under `web/styles/`, `web/src/*.generated.ts`, `web/assets/`, `web/llms.txt` and
`web/tokens.json` is generated by `scripts/web-*.main.kts` and committed so `token_drift.yml` can
diff it. Never hand-edit it — change the converter, or change the value in Figma. Build output goes
to `web/dist/` only; a build that dirties a committed path is a bug.

Those scripts need **Kotlin 2.3.20** (newer crashes `.main.kts`), and `const val` does not compile
