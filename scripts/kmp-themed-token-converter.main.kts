...
private fun buildThemedInterfaceCode(
    ...
    return buildString {
        ...
        appendLine()
        appendLine("/**")
        appendLine(" * The themed palette a component is styled from: a [ThemedHue] selects the hue's solid")
        appendLine(" * palette, and [ThemedHue.subtle] its subtle one. A component takes it as one parameter")
        appendLine(" * and reads the slots it needs from `LemonadeTheme.themed[style]`.")
        append(defaultAutoGenerationMessage(scriptFilePath = scriptFilePath))
        appendLine(" */")
        appendLine("@Immutable")
        appendLine("@ExperimentalLemonadeApi")
        appendLine("public sealed interface ThemedStyle")
        appendLine()
        appendLine("/**")
        appendLine(" * Names one hue of [LemonadeThemedColors]. As a [ThemedStyle] it selects the hue's solid")
        appendLine(" * palette; [subtle] selects its subtle one.")
        append(defaultAutoGenerationMessage(scriptFilePath = scriptFilePath))
        appendLine(" */")
        appendLine("@ExperimentalLemonadeApi")
        appendLine("public enum class ThemedHue : ThemedStyle {")
        primary.keys.forEach { hue -> appendLine("    $hue,") }
        appendLine("    ;")
        appendLine()
        appendLine("    /** This hue's subtle palette. */")
        appendLine("    public val subtle: ThemedStyle = SubtleThemedStyle(hue = this)")
        appendLine("}")
        appendLine()
        appendLine("internal data class SubtleThemedStyle(")
        appendLine("    val hue: ThemedHue,")
        appendLine(") : ThemedStyle")
        appendLine()
        appendLine("/**")
        ...
        appendLine(" */")
        appendLine("@ExperimentalLemonadeApi")
        appendLine("public operator fun LemonadeThemedColors.get(style: ThemedStyle): ThemedColor =")
        appendLine("    when (style) {")
        appendLine("        is ThemedHue -> get(hue = style)")
        appendLine("        is SubtleThemedStyle -> get(hue = style.hue).subtle")
        appendLine("    }")
        appendLine()
        appendLine("/**")
        ...
        appendLine(" */")
        appendLine("@ExperimentalLemonadeApi")
        appendLine("@Suppress(\"CyclomaticComplexMethod\")")
        appendLine("public operator fun LemonadeThemedColors.get(hue: ThemedHue): ThemedPrimaryColor =")
        appendLine("    when (hue) {")
        primary.keys.forEach { hue -> appendLine("        ThemedHue.$hue -> ${hue.sanitizedValueName()}") }
        appendLine("    }")
    }
}
