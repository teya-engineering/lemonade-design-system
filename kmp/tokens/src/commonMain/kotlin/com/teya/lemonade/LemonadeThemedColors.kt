package com.teya.lemonade

import androidx.compose.runtime.Immutable
import androidx.compose.ui.graphics.Color

/**
 * Themed color tokens from Lemonade DS - Foundations.
 *
 * A generic, theme-aware hue palette sitting between the primitive ramps and the
 * semantic tokens in [LemonadeSemanticColors]. Use these when a colour carries
 * application meaning that the design system does not model - chart series,
 * categories, per-role or per-status accents - instead of reaching for
 * [LemonadePrimitiveColors], which is not theme-aware.
 *
 * Every hue is a [ThemedPrimaryColor], carrying a saturated palette and a
 * [ThemedPrimaryColor.subtle] one, so a component can hold either as a [ThemedColor]
 * and style itself from it. Hues share one type deliberately: they differ in value,
 * not in shape, so a chart series or a per-role mapping can hold them together.
 *
 * Prefer a semantic token whenever one fits.
 *
 * Experimental: the shape of this layer is still settling. Opt in with
 * `@OptIn(ExperimentalLemonadeApi::class)` and expect to revisit call sites.
 * Auto-generated content.
 *
 * ⚠️ **DO NOT MODIFY THIS FILE MANUALLY** ⚠️
 *
 * This file is automatically generated by the scripts/kmp-themed-token-converter.main.kts script.
 * All changes will be overwritten when the script is run again.
 *
 * To add updated content:
 * 1. Add your file to the tokens/ directory
 * 2. Run: kotlin scripts/kmp-themed-token-converter.main.kts
 *
 * @generated by scripts/kmp-themed-token-converter.main.kts
 */
@ExperimentalLemonadeApi
public interface LemonadeThemedColors {
    public val amber: ThemedPrimaryColor
    public val blue: ThemedPrimaryColor
    public val cyan: ThemedPrimaryColor
    public val fuchsia: ThemedPrimaryColor
    public val green: ThemedPrimaryColor
    public val greenLime: ThemedPrimaryColor
    public val indigo: ThemedPrimaryColor
    public val neutral: ThemedPrimaryColor
    public val orange: ThemedPrimaryColor
    public val pink: ThemedPrimaryColor
    public val purple: ThemedPrimaryColor
    public val red: ThemedPrimaryColor
    public val rose: ThemedPrimaryColor
    public val teal: ThemedPrimaryColor
    public val violet: ThemedPrimaryColor
    public val yellow: ThemedPrimaryColor
    public val yellowLime: ThemedPrimaryColor
}

/**
 * The slots every themed palette provides, saturated or subtle alike, so one can be
 * passed around as a value - a chart series, a per-role accent, a category.
 *
 * ```
 * val style: ThemedColor = if (subtle) themed.blue.subtle else themed.blue
 * Box(Modifier.background(style.background).border(1.dp, style.border))
 * ```
 * Auto-generated content.
 *
 * ⚠️ **DO NOT MODIFY THIS FILE MANUALLY** ⚠️
 *
 * This file is automatically generated by the scripts/kmp-themed-token-converter.main.kts script.
 * All changes will be overwritten when the script is run again.
 *
 * To add updated content:
 * 1. Add your file to the tokens/ directory
 * 2. Run: kotlin scripts/kmp-themed-token-converter.main.kts
 *
 * @generated by scripts/kmp-themed-token-converter.main.kts
 */
@ExperimentalLemonadeApi
public interface ThemedColor {
    public val background: Color
    public val border: Color
    public val onBackground: Color
}

/**
 * A hue's saturated palette. Adds the slots that only make sense on the hue itself,
 * plus its [subtle] counterpart.
 * Auto-generated content.
 *
 * ⚠️ **DO NOT MODIFY THIS FILE MANUALLY** ⚠️
 *
 * This file is automatically generated by the scripts/kmp-themed-token-converter.main.kts script.
 * All changes will be overwritten when the script is run again.
 *
 * To add updated content:
 * 1. Add your file to the tokens/ directory
 * 2. Run: kotlin scripts/kmp-themed-token-converter.main.kts
 *
 * @generated by scripts/kmp-themed-token-converter.main.kts
 */
@ExperimentalLemonadeApi
public interface ThemedPrimaryColor : ThemedColor {
    public val backgroundHigh: Color
    public val content: Color
    public val contentInverse: Color
    public val onBackgroundHigh: Color
    public val subtle: ThemedColor
}

/**
 * The themed palette a component is styled from: a [ThemedHue] selects the hue's solid
 * palette, and [ThemedHue.subtle] its subtle one. A component takes it as one parameter
 * and reads the slots it needs from `LemonadeTheme.themed[style]`.
 * Auto-generated content.
 *
 * ⚠️ **DO NOT MODIFY THIS FILE MANUALLY** ⚠️
 *
 * This file is automatically generated by the scripts/kmp-themed-token-converter.main.kts script.
 * All changes will be overwritten when the script is run again.
 *
 * To add updated content:
 * 1. Add your file to the tokens/ directory
 * 2. Run: kotlin scripts/kmp-themed-token-converter.main.kts
 *
 * @generated by scripts/kmp-themed-token-converter.main.kts
 */
@Immutable
@ExperimentalLemonadeApi
public sealed interface ThemedStyle

/**
 * Names one hue of [LemonadeThemedColors]. As a [ThemedStyle] it selects the hue's solid
 * palette; [subtle] selects its subtle one.
 * Auto-generated content.
 *
 * ⚠️ **DO NOT MODIFY THIS FILE MANUALLY** ⚠️
 *
 * This file is automatically generated by the scripts/kmp-themed-token-converter.main.kts script.
 * All changes will be overwritten when the script is run again.
 *
 * To add updated content:
 * 1. Add your file to the tokens/ directory
 * 2. Run: kotlin scripts/kmp-themed-token-converter.main.kts
 *
 * @generated by scripts/kmp-themed-token-converter.main.kts
 */
@ExperimentalLemonadeApi
public enum class ThemedHue : ThemedStyle {
    Amber,
    Blue,
    Cyan,
    Fuchsia,
    Green,
    GreenLime,
    Indigo,
    Neutral,
    Orange,
    Pink,
    Purple,
    Red,
    Rose,
    Teal,
    Violet,
    Yellow,
    YellowLime,
    ;

    /** This hue's subtle palette. */
    public val subtle: ThemedStyle = SubtleThemedStyle(hue = this)
}

internal data class SubtleThemedStyle(
    val hue: ThemedHue,
) : ThemedStyle

/**
 * The palette [style] selects.
 * Auto-generated content.
 *
 * ⚠️ **DO NOT MODIFY THIS FILE MANUALLY** ⚠️
 *
 * This file is automatically generated by the scripts/kmp-themed-token-converter.main.kts script.
 * All changes will be overwritten when the script is run again.
 *
 * To add updated content:
 * 1. Add your file to the tokens/ directory
 * 2. Run: kotlin scripts/kmp-themed-token-converter.main.kts
 *
 * @generated by scripts/kmp-themed-token-converter.main.kts
 */
@ExperimentalLemonadeApi
public operator fun LemonadeThemedColors.get(style: ThemedStyle): ThemedColor =
    when (style) {
        is ThemedHue -> get(hue = style)
        is SubtleThemedStyle -> get(hue = style.hue).subtle
    }

/**
 * The palette for [hue].
 * Auto-generated content.
 *
 * ⚠️ **DO NOT MODIFY THIS FILE MANUALLY** ⚠️
 *
 * This file is automatically generated by the scripts/kmp-themed-token-converter.main.kts script.
 * All changes will be overwritten when the script is run again.
 *
 * To add updated content:
 * 1. Add your file to the tokens/ directory
 * 2. Run: kotlin scripts/kmp-themed-token-converter.main.kts
 *
 * @generated by scripts/kmp-themed-token-converter.main.kts
 */
@ExperimentalLemonadeApi
@Suppress("CyclomaticComplexMethod")
public operator fun LemonadeThemedColors.get(hue: ThemedHue): ThemedPrimaryColor =
    when (hue) {
        ThemedHue.Amber -> amber
        ThemedHue.Blue -> blue
        ThemedHue.Cyan -> cyan
        ThemedHue.Fuchsia -> fuchsia
        ThemedHue.Green -> green
        ThemedHue.GreenLime -> greenLime
        ThemedHue.Indigo -> indigo
        ThemedHue.Neutral -> neutral
        ThemedHue.Orange -> orange
        ThemedHue.Pink -> pink
        ThemedHue.Purple -> purple
        ThemedHue.Red -> red
        ThemedHue.Rose -> rose
        ThemedHue.Teal -> teal
        ThemedHue.Violet -> violet
        ThemedHue.Yellow -> yellow
        ThemedHue.YellowLime -> yellowLime
    }
